#!/usr/bin/env bash
# avyra-populate-bakeoff2.sh — add LTX-2.5 distilled, Wan 2.2 Fun-Control-Camera HIGH, ostris orbit LoRAs, H3 8-step LoRA (volume resized to 350 GB, nothing removed)
...
# /workspace/logs/populate-bakeoff2.log on the volume.
# Already on both volumes and reused: Wan2_1_VAE_bf16, umt5-xxl-enc-bf16, lightx2v_I2V_14B_480p (Wan 2.1) LoRA.
set -uo pipefail
V=/workspace; M="$V/ComfyUI/models"; mkdir -p "$V/logs"; exec > >(tee -a "$V/logs/populate-bakeoff2.log") 2>&1
...
get(){
  ...
  printf 'bakeoff2 %10s  %s\n' "$(numfmt --to=iec "$have")" "$dest/$(basename "$file")" >> "$V/MANIFEST-bakeoff2.txt"
}
...
if [ -n "${VOL_GB:-}" ] && [ "$(( ${used_gb:-0} + NEED_GB ))" -gt "$(( VOL_GB - 2 ))" ]; then
  echo "POPULATE_BAKEOFF_FAILED (no space: used ${used_gb} + ${NEED_GB} > ${VOL_GB} GB)"; sleep 600; exit 1
fi
say "1/3 LTX-2.5 distilled (comfyicu mirror, ungated): transformer int8 21.5 GB, Gemma-4-12B text encoder int8 15.4 GB, VAEs, spatial upscaler"
get comfyicu/LTX-2.5 "diffusion_models/ltx-2.5-22b-distilled-transformer-comfy-int8-convrot.safetensors" diffusion_models
...
say "2/3 Wan 2.2 Fun-Control-Camera HIGH expert (kijai fp8, 15.5 GB) + ostris orbit LoRAs + H3 8-step turbo LoRA"
get Kijai/WanVideo_comfy_fp8_scaled "Fun/Wan2_2-Fun-Control-Camera-A14B-HIGH_fp8_e4m3fn_scaled_KJ.safetensors" diffusion_models
get ostris/wan22_i2v_14b_orbit_shot_lora "wan22_14b_i2v_orbit_high_noise.safetensors" loras
get ostris/wan22_i2v_14b_orbit_shot_lora "wan22_14b_i2v_orbit_low_noise.safetensors" loras
get Comfy-Org/MiniMax-H3 "loras/minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors" loras
say "3/3 manifest + space: used $(du -sBG "$V" 2>/dev/null | cut -f1) of ${VOL_GB:-?} GB"
if [ "$FAILS" -eq 0 ]; then echo "POPULATE_MOTION_OK"; sleep 20; runpodctl remove pod "$RUNPOD_POD_ID" 2>/dev/null || true
else echo "POPULATE_BAKEOFF_FAILED ($FAILS)"; sleep 600; fi
