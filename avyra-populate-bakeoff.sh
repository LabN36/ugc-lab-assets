#!/usr/bin/env bash
# avyra-populate-bakeoff.sh — add MiniMax H3 (ComfyUI repack, int8 convrot + turbo LoRAs) for the open-model bake-off
...
# /workspace/logs/populate-bakeoff.log on the volume.
# Already on both volumes and reused: Wan2_1_VAE_bf16, umt5-xxl-enc-bf16, lightx2v_I2V_14B_480p (Wan 2.1) LoRA.
set -uo pipefail
V=/workspace; M="$V/ComfyUI/models"; mkdir -p "$V/logs"; exec > >(tee -a "$V/logs/populate-bakeoff.log") 2>&1
...
get(){
  ...
  printf 'bakeoff %10s  %s\n' "$(numfmt --to=iec "$have")" "$dest/$(basename "$file")" >> "$V/MANIFEST-bakeoff.txt"
}
...
say "1/2 MiniMax H3 (Comfy-Org repack): fl2va transformer int8 convrot 21 GB, Qwen3-VL-32B text encoder int8 27 GB, VAEs, turbo LoRAs"
get Comfy-Org/MiniMax-H3 "diffusion_models/minimax_h3_fl2va_pruned_int8_convrot.safetensors" diffusion_models
get Comfy-Org/MiniMax-H3 "text_encoders/qwen3vl_32b_minimax_h3_int8_convrot.safetensors" text_encoders
get Comfy-Org/MiniMax-H3 "vae/minimax_h3_video_vae_fp16.safetensors" vae
get Comfy-Org/MiniMax-H3 "vae/minimax_h3_audio_vae_fp32.safetensors" vae
get Comfy-Org/MiniMax-H3 "loras/minimax_h3_fl2v_turbo_4step_v1.0_768p_comfyui_bf16.safetensors" loras
get lightx2v/Minimax-h3-Turbo "minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors" loras
say "2/2 manifest + space: $(df -h "$V" | tail -1); used $(du -sBG "$V" 2>/dev/null | cut -f1)"
if [ "$FAILS" -eq 0 ]; then echo "POPULATE_MOTION_OK"; sleep 20; runpodctl remove pod "$RUNPOD_POD_ID" 2>/dev/null || true
else echo "POPULATE_BAKEOFF_FAILED ($FAILS)"; sleep 900; fi
